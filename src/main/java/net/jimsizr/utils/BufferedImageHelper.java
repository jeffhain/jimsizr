...
public class BufferedImageHelper {
    ...
    public enum BihPixelFormat {
        ...
        public int toArgb32FromPixel(int pixel) {
            ...
            switch (this) {
                case ABGR32:
                case XBGR24: ret = JisColorUtils.toArgb32FromAbgr32(ret); break;
                case RGBA32: ret = JisColorUtils.toArgb32FromRgba32(ret); break;
                case BGRA32: ret = JisColorUtils.toArgb32FromBgra32(ret); break;
                default: break;
            }
            if (!this.hasAlpha) {
        ...
        public int toPixelFromArgb32(int argb32) {
            ...
            switch (this) {
                case ABGR32:
                case XBGR24: ret = JisColorUtils.toAbgr32FromArgb32(ret); break;
                case RGBA32: ret = JisColorUtils.toRgba32FromArgb32(ret); break;
                case BGRA32: ret = JisColorUtils.toBgra32FromArgb32(ret); break;
                default: break;
            }
            return ret;
        }
    ...
    public void getPixelsInto(
        ...
        JisUtils.requireNonNull(color32Arr);
        // Implicit null check.
        ...
        JisUtils.requireSupOrEq(width, dw, "color32ArrScanlineStride");
        JisUtils.requireInRange(0, dw - 1, dstX, "dstX");
        JisUtils.requireInRange(0, dw - dstX, width, "width");
        final int dh = computeArrayHeight(
            color32Arr.length,
            color32ArrScanlineStride,
            dstX,
            width);
        JisUtils.requireInRange(0, dh - 1, dstY, "dstY");
        JisUtils.requireInRange(0, dh - dstY, height, "height");
    ...
    public void setPixelsFrom(
        ...
        JisUtils.requireSupOrEq(width, sw, "color32ArrScanlineStride");
        JisUtils.requireInRange(0, sw - 1, srcX, "srcX");
        JisUtils.requireInRange(0, sw - srcX, width, "width");
        final int sh = computeArrayHeight(
            color32Arr.length,
            color32ArrScanlineStride,
            srcX,
            width);
        JisUtils.requireInRange(0, sh - 1, srcY, "srcY");
        JisUtils.requireInRange(0, sh - srcY, height, "height");
    ...
    public static void copyImage(
        ...
        JisUtils.requireInRange(0, maxXRange, width, "width");
        JisUtils.requireInRange(0, maxYRange, height, "height");
    ...
    private int convertDataToArgb32_colorModel(
        ...
        if (premul) {
            argb32 = JisColorUtils.toPremulAxyz32(argb32);
        }
        return argb32;
    }
    ...
    private int convertPixelToArgb32_INT_BIH_PIXEL_FORMAT(
        ...
        if (premul != this.image.isAlphaPremultiplied()) {
            if (premul) {
                ...
                argb32 = JisColorUtils.toNonPremulAxyz32(argb32);
            }
        }
        return argb32;
    }
    ...
    private int convertPixelToArgb32_USHORT_555_RGB(
        ...
        int argb32 = JisColorUtils.toAbcd32_noCheck(0xFF, r8, g8, b8);
        if (premul) {
            argb32 = JisColorUtils.toPremulAxyz32(argb32);
        }
        return argb32;
            ...
            for (int j = 0; j < height; j++) {
                ...
                if (needFormatRework) {
                    ...
                    for (int i = width; --i >= 0;) {
                        ...
                        if (dstPremul) {
                            if (isAlphaInMSByte) {
                                pixel = JisColorUtils.toPremulAxyz32(pixel);
                            } else {
                                pixel = JisColorUtils.toPremulXyza32(pixel);
                            }
                        } else {
                            if (isAlphaInMSByte) {
                                pixel = JisColorUtils.toNonPremulAxyz32(pixel);
                            } else {
                                pixel = JisColorUtils.toNonPremulXyza32(pixel);
                            }
                        }
                        dstArr[index] = pixel;
                    }
    ...
    private void checkRectInImage(
        ...
        JisUtils.requireInRange(0, iw - 1, x, xName);
        JisUtils.requireInRange(0, ih - 1, y, yName);
        JisUtils.requireInRange(0, iw - x, width, widthName);
        JisUtils.requireInRange(0, ih - y, height, heightName);
    }
